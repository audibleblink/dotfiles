...
# Unset options
rg -Io 'set\s+-[aFgopqsuUw]+\s+"?@([^\s]+(\w|_))"?' -r '@$1' $XDG_DATA_HOME/tmux/plugins/tmux/**/*.conf | uniq | xargs -n1 -P0 tmux set -Ugq

status_dir="$XDG_DATA_HOME/tmux/plugins/tmux/status"
status_utils_file="$XDG_DATA_HOME/tmux/plugins/tmux/utils/status_module.conf"

# Get all status modules in $XDG_DATA_HOME/tmux/plugins/tmux/status/
modules=()

...
OP_SUBDOMAIN = "my"
CMD_PASSWORD_PROMPT = (
    "rofi -password -dmenu -p 'Vault Password' -l 0 -sidebar"
)
TFA_PASSWORD_PROMPT = (
    "rofi -password -dmenu -p 'Vault 2FA' -l 0 -sidebar"
)
...
class OnePass:
    ...
    def login(cls):
        ...
        try:
            op = pexpect.spawn("op signin --output=raw")
            op.expect("Enter the password for virtualsnyper@gmail.com at my.1password.com: ")
            op.sendline(password)
            out = op.read_nonblocking(128, timeout=1).decode()

            if "six-digit" in out:
                token = execute_command(TFA_PASSWORD_PROMPT)
                op.sendline(token)

            
            session_id = op.read_nonblocking(128).decode().strip()
            # session_id = execute_command(
            #     CMD_OP_LOGIN.format(password=password, tfa=token, subdomain=OP_SUBDOMAIN)
            # )
        except ExecuteError:

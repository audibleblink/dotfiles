...
if [[ $percentage -lt 6 ]]; then
  ...
  output="$EMPTY_HEART$EMPTY_HEART$EMPTY_HEART$EMPTY_HEART$EMPTY_HEART"
elif [[ $percentage -ge 95 ]]; then
  # 95-100%: Show five full hearts
  output="$FULL_HEART$FULL_HEART$FULL_HEART$FULL_HEART$FULL_HEART"
else
...
sketchybar --set "$NAME" icon="${output}" label="${percentage}%" icon.color=$RED
